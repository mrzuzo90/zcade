...
- `src/io/persistence.ts` — `serializeProject()`/`serializeProjectToJSON()` (reads `useCanvasStore`/`useWireStore` state, in insertion order, via `GRID_SIZE` for `meta.gridSize`), `parseZcadeJSON()` (JSON.parse + validate, same never-throws contract), and `loadProject()` (applies an already-validated file: replaces canvas/wire content wholesale, stops any running simulation first, and clears undo history — a loaded file is a fresh baseline, not something to undo back out of). Deliberately does **not** touch view state (scale/position/grid/snap) on load — not part of the schema, and resetting the user's pan/zoom on every load would be surprising.
- `src/store/canvas.ts` / `src/store/wires.ts` — new `loadComponents()`/`loadWires()` actions, the store-level piece `loadProject()` needed and neither store had before this session (their old `resetEditorStores()`-style test-only `setState()` calls only ever cleared to empty). Both bypass the command/history layer entirely (same "file load isn't a user edit to undo" reasoning `history.clear()`'s own pre-existing doc comment already called out), preserve loaded ids **verbatim** (wires reference components by id, so ids are never regenerated on load), and bump the module's internal id-generator counter past the highest numeric suffix seen in the loaded ids — so a subsequent `addComponent`/`completeWire` call in the same session can't mint an id that collides with a restored one. This is the one piece of this work that reaches into SOLV/ROUTE-adjacent files (`canvas.ts`/`wires.ts` aren't exclusively owned by either, per the Week 1 directory map) — additive-only, no existing action's behavior changed.
- `src/io/fileSystem.ts` — native open/save via `@tauri-apps/plugin-dialog` + `@tauri-apps/plugin-fs`, gated by `isTauri()` (re-exported from `@tauri-apps/api/core` — the officially supported runtime-detection call, not a hand-rolled `'__TAURI__' in window` sniff) with a browser fallback (`<input type="file">` + FileReader for open; a Blob + anchor-`download` for save) so `npm run dev` in Chrome keeps working with zero Tauri runtime present. `writeProjectFile()` (plain "Save" to an already-known path, no dialog) and `readProjectFile()` (reopen a recent-files entry by path, no dialog) both carry an explicit doc-comment flag: Tauri only auto-grants fs scope for a path a dialog *just* returned in the same session — a path restored from a previous session's app-state/recent-files has no such grant on a fresh launch, so these can legitimately reject with a permission error even though the path is "known". Every call site that uses them (`src/store/project.ts`'s `saveProject()`/`openRecentFile()`) catches that and falls back to re-prompting via the dialog, which re-establishes scope.
- `src/io/autosave.ts` — a 30s (`AUTOSAVE_INTERVAL_MS`) `setInterval` writing `serializeProjectToJSON()` to `autosave.zcade.json` in the Tauri app-data directory (`BaseDirectory.AppData`); every exported function is a silent no-op under `isTauri() === false` (never a thrown error) — "no native shell" is a supported mode per this doc's Offline-First section, not a degraded one. `getAutosaveMtimeMs()` (via plugin-fs's `stat()`) is what powers the recovery check below.
- `src/io/appState.ts` — a small `app-state.json` (Tauri app-data dir) / `localStorage` (browser, key `zcade.appState.v1`) record of `recentFiles`/`lastFilePath`/`lastManualSaveMs`, so the recent-files list and the autosave-recovery comparison both survive an app restart (Tauri) or a page reload (browser dev). Corrupt/unreadable state is treated exactly like a bad `.zcade` file — falls back to a clean slate rather than blocking startup.
- `src/store/project.ts` — `useProjectStore`, the orchestration layer a "File" menu hangs off: `init()` (loads app-state, starts autosave, and flags `pendingRecovery` when an autosave's mtime is newer than `lastManualSaveMs` — Tauri only), `newProject()`, `openProject()`/`openRecentFile()`, `saveProject()` (overwrites the known path directly, falling back to `saveProjectAs()` on any failure — see the fs-scope caveat above) / `saveProjectAs()` (always re-prompts), `recoverAutosave()`/`dismissRecovery()`. `recentFiles` is capped at `MAX_RECENT_FILES = 10`, most-recent-first, de-duplicated by path.
- `src/components/Canvas/Toolbar.tsx` / `src/App.tsx` — a minimal but functional File menu (Nuevo/Abrir/Guardar/Guardar como/Recientes, all disabled while `isRunning` same as the existing edit-disabling controls) and an amber recovery banner (`RecoveryBanner`) shown whenever `pendingRecovery` is set, with Recuperar/Descartar actions. `useProjectStore.getState().init()` is called once from `App`'s top-level `useEffect`.
- `src-tauri/Cargo.toml` / `src-tauri/src/lib.rs` / `src-tauri/capabilities/default.json` — added the `tauri-plugin-dialog`/`tauri-plugin-fs` crates (and their `@tauri-apps/plugin-dialog`/`@tauri-apps/plugin-fs` JS counterparts), registered both plugins in `lib.rs`, and granted `dialog:default` + a set of `fs:*` app-data-scoped permissions in the default capability. `cargo check` passes in this worktree, but **no native `npm run tauri dev` window was exercised this session** (same caveat Phase 1's write-up already carried) — the exact runtime behavior of Tauri's per-path fs scope (the `writeProjectFile`/`readProjectFile` caveat above) is reasoned from the plugin's documented behavior, not observed end-to-end in a real window. Flagged as the one meaningfully unverified piece of this work.
- `tests/integration/persistence-roundtrip.test.ts` — un-skipped and implemented per its own pre-existing assertion-plan doc comment: builds `buildDOL()`, serializes through the real JSON text round-trip (not just the in-memory object), resets both stores, reloads, and asserts the reloaded `components`/`wires` are identical (ids are preserved verbatim, so this is a direct equality check, not a structural one) *and* that driving an identical `setPressed` sequence against the reloaded circuit produces the exact same `componentStates` trajectory tick-by-tick as the same circuit rebuilt fresh — "identical simulation behavior", not just identical static data. Also covers a rejected malformed/foreign-JSON load leaving store state untouched, and an explicit `wire.points` override (ROUTE's future waypoint field) surviving the round-trip untouched, per this session's "round-trip whatever fields exist on `Wire` objects generically" constraint.
- `tests/io/schema.test.ts`, `tests/io/persistence.test.ts`, `tests/store/project-store.test.ts` — new unit coverage: schema validation error messages (17 cases), `serializeProject()`/`loadProject()` behavior including the id-collision-avoidance bump (11 cases), and `useProjectStore`'s orchestration logic with every `src/io/*` boundary mocked via `vi.mock` (15 cases) — the first use of `vi.mock` in this codebase, added because exercising `openProject()`/`saveProject()` for real would mean driving a real `<input type="file">` `change` event (not otherwise triggerable in jsdom) or a real Tauri runtime.

**Key design decisions**:
- **Component/wire ids are preserved verbatim on load, never regenerated.** The `.zcade` schema's own wire model references components by id (`WireEndpoint.componentId`); regenerating ids on load would require rewriting every wire's endpoints in lockstep with whatever remapping the loader chose, for zero benefit. The tradeoff is the id-generator-bump mechanism above, which is simple and covered by tests.
- **View state (scale/position/showGrid/snapEnabled) is excluded from the `.zcade` schema and untouched by `loadProject()`.** Mirrors this doc's existing "view state ... deliberately NOT wrapped as commands" call-out for undo history (Phase A Week 1, CORE) — pan/zoom is a session/viewport concern, not project content.
- **`saveProject()`'s direct-overwrite-then-fallback-to-dialog pattern**, rather than either (a) always re-prompting or (b) assuming a known path is always writable, is what makes "Save" behave like a normal desktop app's Ctrl+S *when Tauri's fs scope happens to allow it* while never hard-failing when it doesn't (a restored path from a previous session almost certainly won't have scope yet). This is the single biggest unverified assumption in this session's work — flagged above.
- **Autosave/recovery is Tauri-only, by design, not by oversight.** The browser fallback has no durable per-origin storage story that would survive a real app "restart" (a page reload isn't equivalent to a native app relaunching a lost session) worth building an autosave/recovery UX around; `localStorage` is used only for the much smaller recent-files/last-save bookkeeping (`appState.ts`), which degrades gracefully (list just doesn't persist) if unavailable.
