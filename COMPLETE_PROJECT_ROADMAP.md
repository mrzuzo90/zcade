...
### Deliverables
1. **Symbol system**: JSON symbol metadata schema; SVG→Konva rendering pipeline (crisp at 0.1×–10× zoom); **25+ Tier 1 IEC 60617 symbols** (3φ/1φ/DC sources, 3P/4P contactors, NO/NC aux blocks, magnetothermal breaker 3P, thermal overload relay, NO/NC push buttons, latching emergency stop, 3-wire and 6-wire 3φ motors, signal lamps ×5 colors, terminals); migration of the 6 existing components.
...
5. **Solver extensions** (required by the canonical circuits — see gap analysis below): cross-instance linked contacts (`linkedTo` tag: aux block "KM1" follows contactor KM1's coil), aux contacts on contactor definitions (13-14 NO, 21-22 NC), thermal overload with 95-96 trip contact + manual trip toggle, latching e-stop, **basic TON timer** (pulled forward from Tier 2 — the Y-Δ canonical circuit is impossible without it), 6-wire motor with Y-Δ semantics, L-L short *detection* (flag, not physics — needed to validate interlock circuits).
...
- W2 D6-8: Remaining 17+: thermal overload, e-stop, aux blocks NO/NC, contactor 4P,
           1φ source, 6-wire motor, lamp colors, terminals, TON timer symbol.
